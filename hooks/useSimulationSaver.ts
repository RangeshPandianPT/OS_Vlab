...
import {
  saveSimulation as firebaseSaveSimulation,
  loadSimulation,
  loadUserSimulations,
  deleteSimulation as firebaseDeleteSimulation,
  updateSimulation as firebaseUpdateSimulation,
...
export function useSavedSimulations(
  ...
  const saveSimulation = useCallback(
    async (
      ...
      try {
        const simulationId = await firebaseSaveSimulation(
          currentUser,
          algorithmType,
          name,
          simulationState,
  ...
  const deleteSimulation = useCallback(
    async (simulationId: string): Promise<void> => {
      ...
      try {
        await firebaseDeleteSimulation(currentUser, simulationId);
        // Update local state
        setSimulations((prev) =>
  ...
  const updateSimulation = useCallback(
    async (
      ...
      try {
        await firebaseUpdateSimulation(currentUser, simulationId, updates);
        // Update local state
        setSimulations((prev) =>
          prev.map((sim) =>
