...
import { Plus, Trash2, Link, Unlink, ShieldCheck, RotateCcw, AlertTriangle, Info, CheckCircle, XCircle, Zap, Save, BookOpen } from 'lucide-react';
        ...
        <Modal isOpen={true} onClose={onClose} title="Add New Resource">
            <div className="space-y-4">
                <div>
                    ...
                    <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., R1, Printer" className="w-full p-2 mt-1 border border-white/40 dark:border-white/10 rounded-lg bg-background" />
                </div>
                <div>
                    <label className="text-sm font-medium">Total Instances</label>
                    <input type="number" min="1" value={instances} onChange={e => setInstances(parseInt(e.target.value) || 1)} className="w-full p-2 mt-1 border border-white/40 dark:border-white/10 rounded-lg bg-background" />
                </div>
                <div className="flex justify-end gap-2 pt-2">
                    <button onClick={onClose} className="bg-background text-text-primary shadow-card hover:text-accent active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg px-4 py-2 text-sm font-bold flex items-center justify-center gap-2">Cancel</button>
                    <button onClick={handleAdd} className="bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg px-4 py-2 uppercase font-bold text-sm flex items-center justify-center gap-2">Add</button>
                </div>
            </div>
        </Modal>
    );
        ...
        <Modal isOpen={true} onClose={onClose} title={title}>
            <div className="space-y-4">
                <div>
                    ...
                    <select value={pId} onChange={e => setPId(e.target.value)} className="w-full p-2 mt-1 border border-white/40 dark:border-white/10 rounded-lg bg-background">
                        {processes.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                </div>
                <div>
                    <label className="text-sm font-medium">Resource</label>
                    <select value={rId} onChange={e => setRId(e.target.value)} className="w-full p-2 mt-1 border border-white/40 dark:border-white/10 rounded-lg bg-background" disabled={type==='release' && availableToRelease.length === 0}>
                        {type === 'request' ? (
                     ...
                     {type==='release' && availableToRelease.length === 0 && <p className="text-xs text-text-muted mt-1">P{pId} has no resources to release.</p>}
                </div>
                <div className="flex justify-end gap-2 pt-2">
                    <button onClick={onClose} className="bg-background text-text-primary shadow-card hover:text-accent active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg px-4 py-2 text-sm font-bold flex items-center justify-center gap-2">Cancel</button>
                    <button onClick={handleConfirm} className="bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg px-4 py-2 uppercase font-bold text-sm flex items-center justify-center gap-2" disabled={!pId || !rId}>Confirm</button>
                </div>
            </div>
        </Modal>
    );
                                ...
                                {Array.from({ length: rCount }).map((_, rIdx) => (
                                    <td key={rIdx} className="p-1">
                                        <input type="number" min="0" value={matrix[pIdx]?.[rIdx] ?? 0}
                                               ...
                                               className="w-14 p-1 border border-white/40 dark:border-white/10 rounded-md bg-transparent focus:ring-1 focus:ring-accent focus:outline-none text-center" />
                                    </td>
                                ))}
                            </tr>
                        ))}
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" min="1" value={pCount} onChange={e => setPCount(parseInt(e.target.value))} className="w-full p-2 mt-1 border border-white/40 dark:border-white/10 rounded-lg bg-background" />
                    </div>
                    <div>
                        <label className="text-sm font-medium">Resources</label>
                        <input type="number" min="1" value={rCount} onChange={e => setRCount(parseInt(e.target.value))} className="w-full p-2 mt-1 border border-white/40 dark:border-white/10 rounded-lg bg-background" />
                    </div>
                </div>
                        ...
                        {Array.from({ length: rCount }).map((_, rIdx) => (
                             <div key={rIdx} className="text-center">
                                ...
                                <input type="number" min="0" value={available[rIdx] ?? 0}
                                       ...
                                       className="w-16 p-1 mt-1 border border-white/40 dark:border-white/10 rounded-md bg-transparent focus:ring-1 focus:ring-accent focus:outline-none text-center" />
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-4">
                    <button onClick={onClose} className="bg-background text-text-primary shadow-card hover:text-accent active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg px-4 py-2 text-sm font-bold flex items-center justify-center gap-2">Cancel</button>
                    <button onClick={handleRun} className="bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg px-4 py-2 uppercase font-bold text-sm flex items-center justify-center gap-2">Run Algorithm</button>
                </div>
            </div>
             </Modal>
    );
        ...
        <Modal isOpen={true} onClose={onClose} title="Banker's Algorithm Results">
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="max-h-80 overflow-y-auto border border-white/40 dark:border-white/10 rounded-lg p-2 bg-background/80">
                        <table className="w-full text-xs text-left">
                            ...
                            <tbody>
                                {result.steps.map(step => (
                                    <tr key={step.step} className="border-t border-white/40 dark:border-white/10">
                ...
                <div className="flex justify-end pt-2">
                     <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-medium bg-gray-100 dark:bg-gray-700/50 hover:bg-muted/80 text-text-primary transition-colors">
                        Close
                    </button>
                </div>
            </div>
...
const ResolveDeadlockModal: React.FC<{
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Resolve Deadlock">
      ...
      <style>{`.btn-secondary { @apply px-4 py-2 bg-muted font-semibold rounded-lg hover:bg-muted/70 transition-colors; }`}</style>
    </Modal>
  );
};
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-3xl md:text-4xl font-bold text-text-primary drop-shadow-[0_1px_1px_#ffffff]">Deadlock Visualization</h1>
        <button
      ...
      <Card className="p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <button
            ...
            className="p-4 rounded-lg bg-panel shadow-recessed hover:from-red-500/20 hover:to-orange-500/20 border border-white/40 dark:border-white/10 transition-all duration-300 text-left group"
          >
            ...
            <p className="text-xs text-text-muted">Classic deadlock scenario with 2 processes</p>
          </button>
          <button
            onClick={() => loadScenario('safe')}
            className="p-4 rounded-lg bg-panel shadow-recessed hover:from-green-500/20 hover:to-emerald-500/20 border border-white/40 dark:border-white/10 transition-all duration-300 text-left group"
          >
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-text-muted">Banker's algorithm with safe sequence</p>
          </button>
          <button
            onClick={() => loadScenario('unsafe')}
            className="p-4 rounded-lg bg-panel shadow-recessed hover:from-amber-500/20 hover:to-yellow-500/20 border border-white/40 dark:border-white/10 transition-all duration-300 text-left group"
          >
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-text-muted">No safe sequence available</p>
          </button>
        </div>
      </Card>
            ...
            <Card className="p-4 sm:p-6">
                 ...
                 <div className="mb-4 p-3 rounded-lg bg-panel shadow-recessed border border-white/40 dark:border-white/10">
                   ...
                   <div className="grid grid-cols-2 gap-2">
                     <button onClick={resetSimulation} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg">
                       <RotateCcw size={16} />
                       <span>Reset</span>
                     </button>
                     <button onClick={() => setIsSaveModalOpen(true)} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg" title="Save current state to database">
                       <Save size={16} />
                       <span>Save</span>
                     </button>
                   </div>
                 ...
                 <div className="mb-4 p-3 rounded-lg bg-panel shadow-recessed border border-white/40 dark:border-white/10">
                   ...
                   <div className="grid grid-cols-2 gap-2">
                     <button onClick={handleAddProcess} className="flex items-center justify-center gap-2 px-3 py-2 bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg text-xs sm:text-sm">
                       <Plus size={16} />
                       <span>Process</span>
                     </button>
                     <button onClick={() => setModal('addResource')} className="flex items-center justify-center gap-2 px-3 py-2 bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg text-xs sm:text-sm">
                       <Plus size={16} />
                       <span>Resource</span>
                     </button>
                   </div>
                 ...
                 <div className="mb-4 p-3 rounded-lg bg-panel shadow-recessed border border-white/40 dark:border-white/10">
                   ...
                   <button onClick={() => setModal('banker')} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg text-xs sm:text-sm">
                     <ShieldCheck size={16} />
                 ...
                 {deadlockedInfo.cycles.length > 0 && (
                     <div className="p-3 sm:p-4 rounded-lg flex items-start sm:items-center gap-3 bg-panel shadow-recessed border-2 border-red-500 animate-pulse">
                         <AlertTriangle size={24} className="text-text-primary flex-shrink-0" />
                         <div className="flex-grow">
                             <h3 className="font-bold text-sm sm:text-base text-text-primary">Deadlock Detected!</h3>
                             <p className="text-xs text-red-700 dark:text-red-300 mb-2">Circular wait found in resource graph</p>
                             <button onClick={() => setModal('resolve')} className="text-xs sm:text-sm font-semibold px-3 py-2 rounded-md bg-accent text-accent-fg shadow-card hover:brightness-110 active:translate-y-[2px] active:shadow-pressed border border-white/20 transition-all duration-150 rounded-lg hover:from-red-700 hover:to-red-800 transition-all duration-300 shadow-md hover:shadow-lg">
                               Resolve Now
                             </button>
                         </div>
                     </div>
        ...
        <div className="lg:col-span-2">
          <Card className="p-4 sm:p-6 h-[400px] sm:h-[500px] lg:h-[550px]">
            ...
            {processes.length === 0 && resources.length === 0 ? (
              <div className="h-[calc(100%-3rem)] flex flex-col items-center justify-center text-center p-4 sm:p-8 bg-panel rounded-lg border-2 border-dashed border-white/40 dark:border-white/10">
                <AlertTriangle size={48} className="text-gray-400 dark:text-gray-600 mb-4" />
                <h3 className="text-base sm:text-lg font-semibold text-text-primary mb-2">Empty Graph</h3>
              ...
              <div className="h-[calc(100%-3rem)] w-full overflow-auto bg-background rounded-lg border border-white/40 dark:border-white/10 p-2">
      ...
      <SaveSimulationModal
        ...
        defaultName={`Deadlocks Simulation`}
      />
