  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <aside className="lg:col-span-3">
        <div className="bg-background/80 p-4 rounded-2xl border border-white/40 dark:border-white/10">
          <h3 className="text-xs font-semibold uppercase text-text-muted tracking-wider mb-3">Topics</h3>
          ...
          <Card className="p-6" id="fcfs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div>
                ...
                <ul className="list-disc pl-5 text-text-muted mt-2">
            ...
            <p className="mb-3 text-text-muted">This simulation lets you observe the non-preemptive nature and the Convoy Effect of FCFS.</p>
            ...
            <p className="text-text-muted mt-2">Sample process list to see the convoy effect:</p>
...
P3 (Arrival=5, Burst=3)
            ...
            <p className="mt-3 text-text-muted">With FCFS the CPU will run P1 first for the full 24 units and P2/P3 will wait, demonstrating the convoy effect where short jobs are held up by a long job.</p>
          </Card>
        )}
        ...
        {active === 'sjf' && (
          <Card className="p-6" id="sjf">
            ...
            <p className="mb-3 text-text-muted">(See the original simulation notes for detailed loop steps.) Generally:</p>
            <ol className="list-decimal pl-5 space-y-2 text-text-muted">
        ...
        {active === 'priority' && (
          <Card className="p-6" id="priority">
            ...
            <p className="mb-3 text-text-muted">(See original document for detailed loop steps.) In short:</p>
            <ol className="list-decimal pl-5 space-y-2 text-text-muted">
        ...
        {active === 'rr' && (
          <Card className="p-6" id="rr">
            ...
            <p className="mb-3 text-text-muted">Pick a quantum large enough to allow progress but small enough to keep responsiveness. Typical quanta are between 10–100 ms depending on the system. The right value balances fairness and efficiency.</p>

            <h3 className="mt-6 mb-2 text-xl font-semibold">Simulation Steps for Round-Robin Scheduling</h3>
            <p className="mb-3 text-text-muted">(See original doc for detailed steps.) In summary:</p>
            <ol className="list-decimal pl-5 space-y-2 text-text-muted">
        ...
        {active === 'mlq' && (
          <Card className="p-6" id="mlq">
            ...
            <p className="mb-3 text-text-muted">Each queue has its own rules. Queues are set up by priority or allocated time slices. Basic MLQ does not move processes between queues (no mobility).</p>
            ...
            <p className="mb-3 text-text-muted">Like a hospital triage: emergencies are seen first, routine checkups in another queue, and follow-ups in a lower-priority queue.</p>

            <h3 className="mt-6 mb-2 text-xl font-semibold">Notes</h3>
            <p className="mb-3 text-text-muted">MLQ is simple and predictable, but if mobility is desired (processes can change class), consider the Multilevel Feedback Queue (MLFQ) which allows processes to move between queues based on their behavior.</p>
          </Card>
        )}
        ...
        {active === 'process-management' && (
          <Card className="p-6" id="process-management">
            ...
            <p className="mb-3 text-text-muted">A common textbook definition: <em>"A process is a program that is running."</em> A program on disk is static; a process is dynamic and includes the program plus the resources it uses while running.</p>
            ...
            <p className="mt-3 text-text-muted">Each running process has its own memory space to prevent accidental interference and to protect system stability.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Process States</h3>
            <p className="mb-3 text-text-muted">A process moves through different states during its lifetime:</p>
            ...
            <p className="mb-3 text-text-muted">The OS maintains a Process Control Block (PCB) for each process — a data structure that serves as the process's identity card and stores all information needed to manage it. When the CPU switches processes, the current PCB is saved and the next process's PCB is loaded.</p>
            ...
            <p className="mt-3 text-text-muted">The PCB is essential to context switching: saving the outgoing process's PCB and restoring the incoming process's PCB allows execution to resume correctly.</p>
          </Card>
        )}
        ...
        {active === 'process-scheduling' && (
          <Card className="p-6" id="process-scheduling">
            ...
            <p className="mb-3 text-text-muted">To efficiently manage multiple processes, the OS organizes them into different queues. As processes execute they move between these queues depending on their state and needs.</p>
            ...
            <p className="mb-3 text-text-muted">As processes execute they move between these queues — from ready to waiting (when performing I/O), and from waiting back to ready when the event completes. Proper scheduling ensures CPU time is allocated efficiently while keeping I/O and CPU-bound processes balanced.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Context Switch</h3>
            <p className="mb-3 text-text-muted">A context switch occurs when the CPU switches from executing one process to another. The OS saves the state of the outgoing process in its Process Control Block (PCB) and loads the saved state of the incoming process from its PCB so it can resume correctly.</p>

            <ul className="list-disc pl-5 space-y-2 text-text-muted mb-3">
            ...
            <p className="mb-3 text-text-muted">Because context switches consume CPU cycles (saving/restoring registers, updating memory mappings, flushing TLBs in some architectures), good scheduling aims to minimize unnecessary switches while still meeting responsiveness and fairness goals.</p>
        ...
        {active === 'ipc' && (
          <Card className="p-6" id="ipc">
            ...
            <p className="mb-3 text-text-muted">Cooperating processes frequently need to exchange data or coordinate their actions — for example, a text editor and a spell checker working together. The OS provides Interprocess Communication (IPC) mechanisms so processes can communicate and stay in sync.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Two Models of IPC</h3>

            <h4 className="mt-2 font-semibold">1. Shared Memory</h4>
            <p className="mb-2 text-text-muted">A region of memory is mapped into the address space of two or more processes so they can read and write it directly. After the kernel sets up the shared segment, processes access it without kernel mediation — which makes it very fast. Because multiple writers/readers may touch the same data concurrently, proper synchronization (semaphores, mutexes) is required to avoid race conditions.</p>

            <p className="mb-3 text-text-muted"><strong>Example:</strong> In the producer-consumer problem, a producer writes items into a shared buffer and the consumer reads them. Semaphores or mutexes ensure producers don't overwrite unread data and consumers don't read empty slots.</p>

            <h4 className="mt-2 font-semibold">2. Message Passing</h4>
            <p className="mb-2 text-text-muted">Processes exchange data by explicitly sending and receiving messages through OS-provided calls (e.g., send(message), receive()). This model is easier to use across distributed systems (processes on different machines) and provides isolation since memory is not shared directly. It tends to be slower than shared memory due to kernel involvement and copying, but is often safer and simpler to reason about.</p>

            <p className="mb-3 text-text-muted"><strong>Example:</strong> A client process sends a request message to a server process asking for data; the server processes the request and sends back a reply message.</p>
        ...
        {active === 'disk-scheduling' && (
          <Card className="p-6" id="disk-scheduling">
            ...
            <p className="mb-3 text-text-muted">Disk scheduling organizes disk I/O requests so they can be handled efficiently. Moving the disk head (seek time) is the slowest part of I/O. The primary goals are to minimize seek time and maximize throughput (requests completed per second).</p>
            ...
            <p className="mb-2 text-text-muted">Requests are processed in arrival order. Very simple and fair, but can be inefficient: a request far from the current head position may cause long head travel and high average seek time.</p>

            <h4 className="mt-3 font-semibold">2. Shortest Seek Time First (SSTF)</h4>
            <p className="mb-2 text-text-muted">Selects the request closest to the current head position, reducing total head movement. Similar in spirit to SJF for CPU scheduling. Reduces head movement but may cause starvation for distant requests if nearby requests keep arriving.</p>

            <h4 className="mt-3 font-semibold">3. SCAN (Elevator Algorithm)</h4>
            <p className="mb-2 text-text-muted">The disk arm moves in one direction servicing requests until it reaches the end, then reverses direction and continues. Provides fairness because every request in the path gets serviced eventually — analogy: an elevator moving up and down.</p>

            <h4 className="mt-3 font-semibold">4. C-SCAN (Circular SCAN)</h4>
            <p className="mb-2 text-text-muted">Like SCAN but the head only moves in one direction; when it reaches the end it jumps back to the start without servicing requests on the return. Provides more uniform wait times and improved fairness — like a one-way bus route.</p>

            <h4 className="mt-3 font-semibold">5. LOOK and C-LOOK</h4>
            <p className="mb-2 text-text-muted">Variants of SCAN/C-SCAN: the head only goes as far as the last request in a direction rather than traveling to the absolute end. LOOK reverses direction when there are no more requests ahead; C-LOOK jumps back only as far as the first request on the other side. These avoid unnecessary head movement and are more efficient in practice.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Notes and Trade-offs</h3>
            <p className="mb-3 text-text-muted">Choosing the right algorithm depends on workload: FCFS is simple but can have high seek times; SSTF and SCAN-family algorithms aim to reduce head movement at the cost of potential starvation or complexity. LOOK/C-LOOK are practical optimizations to avoid extra travel. Overall, good disk scheduling reduces average seek time and increases throughput.</p>
        ...
        {active === 'memory-management' && (
          <Card className="p-6" id="memory-management">
            ...
            <p className="mb-3 text-text-muted">Memory Management is the part of the OS that tracks and arranges physical memory, decides how much memory to give each process, and ensures isolation and efficient reuse. The primary goals are maximizing memory utilization and protecting processes from each other for stability and security.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Logical vs. Physical Address Space</h3>
            <p className="mb-2 text-text-muted">Each process sees a virtual or logical address space — an abstraction that makes it appear to have its own contiguous memory. The logical (virtual) address is generated by the CPU during execution. The physical address is the real location in RAM.</p>

            <p className="mb-2 text-text-muted">The Memory Management Unit (MMU) in hardware translates logical addresses to physical addresses at runtime. This enables virtual memory, allowing processes to use more apparent memory than physically present by paging swapped pages to disk.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Contiguous Memory Allocation</h3>
            <p className="mb-2 text-text-muted">Contiguous allocation gives each process one continuous block of memory. It's simple to implement and quick to compute addresses, but it suffers from fragmentation.</p>

            <h4 className="mt-2 font-semibold">Fixed Partitioning</h4>
            <p className="mb-2 text-text-muted">Memory is split into fixed-size partitions. Easy but causes internal fragmentation (wasted space inside an allocated partition).</p>

            <h4 className="mt-2 font-semibold">Variable Partitioning</h4>
            <p className="mb-2 text-text-muted">Partitions are sized to fit processes as they arrive. More flexible, but leads to external fragmentation (free holes scattered between allocated blocks).</p>
            ...
            <p className="mt-3 text-text-muted">If external fragmentation becomes severe, the OS can perform compaction (moving processes to coalesce free space) — but compaction is time-consuming.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Paging</h3>
            <p className="mb-2 text-text-muted">Paging eliminates external fragmentation by dividing physical memory into fixed-size frames and logical memory into pages of the same size. The OS keeps a Page Table that maps each page to a frame. When the CPU generates a logical address, the MMU uses the page table to compute the physical address.</p>

            <h4 className="mt-2 font-semibold">Translation Lookaside Buffer (TLB)</h4>
            <p className="mb-2 text-text-muted">To speed up translation, hardware caches recent page→frame mappings in the TLB. A TLB hit yields almost-instant translation. On a TLB miss, the system must consult the page table in memory, which is slower.</p>
        ...
        {active === 'thread-synchronization' && (
          <Card className="p-6" id="thread-synchronization">
            ...
            <p className="mb-3 text-text-muted">When multiple processes or threads run concurrently and access the same data or resources, synchronization ensures accesses are ordered and consistent. Without synchronization, shared data can become corrupt or inconsistent.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">The Critical-Section Problem</h3>
            <p className="mb-2 text-text-muted">The critical section is the portion of code where a process accesses shared resources. The critical-section problem is designing a protocol so that only one process executes its critical section at a time while allowing concurrency elsewhere.</p>
            ...
            <p className="mb-2 text-text-muted">Types of semaphores:</p>
            <ul className="list-disc pl-5 text-text-muted">
            ...
            <p className="mt-3 text-text-muted">Semaphores are flexible but must be used carefully to avoid deadlocks, missed signals, or priority inversion.</p>

            <h3 className="mt-4 mb-2 text-lg font-semibold">Conclusion</h3>
            <p className="text-text-muted">Synchronization primitives like mutexes and semaphores keep concurrent systems consistent. They regulate access to shared data, prevent race conditions, and help keep multi-threaded and multi-process environments stable and correct.</p>
          </Card>
        )}
        ...
        {active === 'deadlocks' && (
          <Card className="p-6" id="deadlocks">
            ...
            <p className="mb-3 text-text-muted">A deadlock occurs when a group of processes are each waiting for resources held by others in the group. Each process holds at least one resource and waits for another; none can proceed, and the system stops making progress.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Coffman Conditions</h3>
            <p className="mb-2 text-text-muted">Four necessary conditions must hold simultaneously for a deadlock to occur:</p>
            <ul className="list-disc pl-5 space-y-2 text-text-muted">
            ...
            <p className="mt-3 text-text-muted">Breaking any one of these conditions prevents deadlock.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Deadlock Prevention</h3>
            <p className="mb-3 text-text-muted">Prevention eliminates one or more Coffman conditions so deadlocks cannot occur. Common approaches include:</p>

            <h4 className="mt-2 font-semibold">Eliminating Hold and Wait</h4>
            <p className="mb-2 text-text-muted">Require processes to request all needed resources at once (or release all before requesting new ones). This prevents processes from holding resources while waiting for others, though it can reduce parallelism and resource utilization.</p>

            <h4 className="mt-2 font-semibold">Eliminating No Preemption</h4>
            <p className="mb-2 text-text-muted">Allow the system to preempt resources: if a process requests a resource that cannot be granted, preempt some resources from that process and give them to others. This only works for resources that can be safely saved and restored.</p>

            <h4 className="mt-2 font-semibold">Eliminating Circular Wait</h4>
            <p className="mb-2 text-text-muted">Impose an ordering on resource types and require processes to request resources in order. This prevents circular chains from forming.</p>

            <p className="mb-3 text-text-muted">Prevention is effective but can be restrictive and reduce concurrency.</p>

            <h3 className="mt-4 mb-2 text-xl font-semibold">Deadlock Avoidance — Banker’s Algorithm</h3>
            <p className="mb-3 text-text-muted">Avoidance algorithms allow requests dynamically but only grant them if the resulting system state is safe. The Banker’s Algorithm treats the OS like a banker: processes declare their maximum needs in advance, and the OS simulates allocations to ensure the system remains in a safe state before granting requests.</p>
        ...
        {active === 'more' && (
          <Card className="p-6" id="more">
            ...
            <p className="text-text-muted">Other scheduling algorithms and OS concepts will be added here. Use the left navigation to jump between topics.</p>
          </Card>
        )}
      </main>
    </div>
