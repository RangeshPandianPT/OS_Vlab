...
/**
 * Serializes a simulation state object into a base64-encoded URL-safe string.
 */
export function serializeState(state: object): string {
  try {
    const json = JSON.stringify(state);
    // btoa works with ASCII; use encodeURIComponent to handle unicode safely
...
/**
 * Builds a full shareable URL for the given page and state.
 * Uses the current window origin + pathname so it works in both dev and prod.
 */
export function buildShareUrl(pageId: Page, state: object): string {
  const encoded = serializeState(state);
  const base = `${window.location.origin}${window.location.pathname}`;
  return `${base}?page=${encodeURIComponent(pageId)}&state=${encoded}`;
}
