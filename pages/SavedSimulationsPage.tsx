...
import type { SavedSimulation } from '../utils/firestoreUtils';
      ...
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <select
            ...
            className="px-3 py-2 border border-border-light dark:border-border-dark rounded-lg bg-transparent text-text-light dark:text-text-dark"
          >
            {algorithmTypes.map(type => (
              <option key={type.value} value={type.value} className="bg-card-light dark:bg-card-dark text-text-light dark:text-text-dark">
                {type.label}
              </option>
            ))}
          </select>
        ...
        {loading ? (
          <div className="flex items-center justify-center p-8">
            ...
            <span className="ml-3 text-text-muted-light dark:text-text-muted-dark">Loading simulations...</span>
          </div>
        ) : filteredSimulations.length === 0 ? (
          <div className="border border-dashed border-border-light dark:border-border-dark rounded-lg p-12 text-center">
            <p className="text-text-muted-light dark:text-text-muted-dark">
              {filterType === 'all' 
              ...
              <Card key={simulation.id} className="p-4 hover:shadow-lg transition-shadow">
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-text-muted-light dark:text-text-muted-dark space-y-1">
                    <p>Created: {simulation.createdAt.toDate().toLocaleDateString()}</p>
                    <p>Updated: {simulation.updatedAt.toDate().toLocaleDateString()}</p>
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted-light dark:text-text-muted-dark">
                  {selectedSimulation.createdAt.toDate().toLocaleDateString()}
                </p>
              </div>
            </div>
            ...
            {selectedSimulation.results && (
              <div>
                ...
                <pre className="text-xs bg-gray-200 dark:bg-gray-800 p-3 rounded overflow-auto max-h-48">
            ...
            <div className="flex gap-2 pt-4">
              <button
                onClick={() => {
                  // In a real implementation, this would navigate to the simulation page
                  // with the saved state loaded
